...
const ViewLeave = () => {
  ...
  const fetchLeaves = async () => {
    try {
      const response = await service.fetchLeave();
  ...
  const handleApprove = async (leave) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await service.approveLeave(
            leave.leave_Id,
            leave.employee_Id
          );
  ...
  const handleReject = async (leave) => {
    if (!rejectionReason) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            const response = await service.rejectLeave(
              leave.leave_Id,
              leave.leave_Action,
              rejectionReason
            );
  ...
  // Handler to capture reason input
      ...
      <section className="ems-content">
        <div className="container">
          <div className="all-employee">
            ...
            {/* {message && <p>{message}</p>} */}
            <div className="hm-rzr-manage-leaves-admin leaves mt-4">
              ...
              {showRejectionReason && (
                <div>
                  <label htmlFor="rejectionReason">
                    {" "}
                    <strong className="me-3">Reason for Rejection:</strong>{" "}
                  </label>
                  <input
                    type="text"
                    id="rejectionReason"
                  ...
                  <button
                    ...
                    onClick={() => handleReject(selectedLeave)} // Same button, but now should trigger the confirmation
                  >
                    Confirm Reject
                  </button>
                </div>
