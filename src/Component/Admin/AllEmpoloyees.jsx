...
const AllEmployees = () => {
  ...
  // Function to fetch users based on teamName and department
  const getAllUsers = async () => {
    try {
      const accessToken = localStorage.getItem("accessToken");

      // Fetching using GET request with query parameters
      ...
      const usersData = response.data.data || []; // Ensure `data` exists and default to empty array
      const departmentsData = [
        ...new Set(usersData.map((user) => user.department).filter(Boolean)),
      ];

      setUsers(usersData); // Set users data safely
      setFilteredUsers(usersData); // Set filtered users safely
      setTotalEmployees(usersData.length); // Total number of employees
  ...
  useEffect(() => {
    getAllUsers(); // Fetch users when teamName or department changes
  }, [teamName, department]);
