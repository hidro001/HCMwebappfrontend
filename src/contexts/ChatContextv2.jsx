...
export function ChatProviderv2({ children }) {
    ...
    async (files, onProgress) => {
      ...
      for (const file of files) {
        ...
        const { success, data } = await new Promise((resolve) => {
          socketRef.current.emit("getPreSignedUploadURL", {
            key: file.name,
        ...
        const confirmResp = await new Promise((res) => {
          socketRef.current.emit("confirmUpload", {
            uniqueKey,
            roomId,
            sender: employeeId,
  ...
  const sendMessageHandler = useCallback(() => {
    console.log("🧠 Inside sendMessageHandler:", { message, activeConversation });
  ...
  const joinAndMarkRoom = useCallback((partnerId) => {
    socketRef.current.emit('joinRoom', { sender: employeeId, receiver: partnerId });
    socketRef.current.emit('markRead', { sender: employeeId, receiver: partnerId });
  }, [employeeId]);
  ...
  const contextValue = useMemo(() => ({
    ...
    employeeId, username, userStatus, selectGroup,
    groups, groupsLoading, groupsError,
    members, memberCount, loadingMembers, membersError,
    conversations, loadingConversations, conversationsError,
    createGroupUIFlow,
    messages, loadingMessages, message, setMessage,
    sendMessageHandler, sendFileHandler, fetchUserGroups,
    loadMembers, selectChat, selectUser, activeConversation, requestFileURL
  ]);
