...
export async function fetchAllRequests() {
  ...
  const response = await axiosInstance.get(
    'https://hmback.onrender.com/api/v1/superadmin/companysettings/requests',
...
export async function deleteRequests(id) {
  try {
    const response = await axiosInstance.delete(
      `https://hmback.onrender.com/api/v1/superadmin/companysettings/requests/${id}`
    );
    return response.data?.data || [];
  } catch (error) {
...
export async function getDepartment() {
  try {
    const response = await axiosInstance.get(
      `https://hmback.onrender.com/api/v1/superadmin/departments`
    );
    return response.data?.data || [];
  } catch (error) {
    console.error('Error deleting request:', error);
