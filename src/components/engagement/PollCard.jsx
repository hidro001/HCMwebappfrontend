...
//       toast.success("Your vote has been recorded!");
//       setHasVoted(true);
//       // Update the feed store instead of refreshing
//       // Assuming the API returns the updated poll
//       const response = await axiosInstance.get(`/polls/${poll._id}`);
//       if (response.data.poll) {
//         useFeedStore.getState().updatePoll(response.data.poll);
//       }
//     } catch (error) {
//       console.error("Error voting on poll:", error);
//       toast.error(
...
//   // Determine if the user has already voted and for which option
//   const userVote = poll.votes.find((vote) => vote.user === userId);
//   const userSelectedOptionId = userVote ? userVote.option : null;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <p className="font-semibold text-lg text-gray-800 dark:text-gray-100">
            {poll.creator?.first_Name || "Unknown"} {poll.creator?.last_Name || "User"} ({poll.creator?.employee_Id || "N/A"})
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400">
