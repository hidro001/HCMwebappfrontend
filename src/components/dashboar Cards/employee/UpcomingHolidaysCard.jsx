...
const UpcomingHolidaysCard = () => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={{ duration: 0.6, ease: [0.4, 0, 0.2, 1] }}
      className="w-full p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-slate-800 shadow-sm hover:shadow-md transition duration-300"
    >
      {/* Header */}
      <div className="flex flex-col md:flex-row items-center justify-between mb-4">
        ...
        {!loading && !errorMsg && holidays.length > 0 && (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={handleRetry}
            className="p-2 rounded-lg text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 mt-4 md:mt-0"
            aria-label="Refresh holidays"
          >
            <FaSync />
          </motion.button>
