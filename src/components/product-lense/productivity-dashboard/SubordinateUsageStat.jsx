...
import { fetchSubordinateFilters } from "../../../service/employeeService";

const SubordinateUsageSection = () => {
  ...
  const { subordinateMostUsedStats, fetchSubordinateMostUsedStats, loading } =
    useUsageStatsStore();

  // Fetch subordinate departments and designations
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        setFiltersLoading(true);
        const { departments, designations } = await fetchSubordinateFilters();
        setDepartments(
          departments.map((dept) => ({ _id: dept, department: dept }))
        );
        setDesignations(
          designations.map((desig) => ({ _id: desig, designation: desig }))
        );
      } catch (error) {
  ...
  // Fetch subordinate usage stats when filters change
  useEffect(() => {
    if (!filtersLoading) {
      fetchSubordinateMostUsedStats(department, designation, 5);
    }
  }, [department, designation, filtersLoading, fetchSubordinateMostUsedStats]);

  // Calculate maximum time for progress bars
  const getMaxTime = () => {
    if (activeTab === "apps" && subordinateMostUsedStats?.topApps?.length > 0) {
      return Math.max(
        ...subordinateMostUsedStats.topApps.map((app) => app.minutes)
      );
    } else if (
      activeTab === "websites" &&
      subordinateMostUsedStats?.topWebsites?.length > 0
    ) {
      return Math.max(
        ...subordinateMostUsedStats.topWebsites.map(
          (website) => website.minutes
        )
      );
    }
    return 100; // Default max time if no data
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              Most used apps and websites among your direct reports
            </p>
          </div>
        ...
        {loading ? (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === "apps" && (
                <>
                  {subordinateMostUsedStats?.topApps?.length > 0 ? (
                    subordinateMostUsedStats.topApps.map((app, index) => (
                      <motion.div
                        key={index}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="text-center py-12">
                      ...
                      <h3 className="mt-4 text-lg font-medium text-gray-500 dark:text-gray-400">
                        No app usage data available for your team
                      </h3>
                    </div>
                  )}
                </>
                ...
                <>
                  {subordinateMostUsedStats?.topWebsites?.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <h3 className="mt-4 text-lg font-medium text-gray-500 dark:text-gray-400">
                        No website usage data available for your team
                      </h3>
                    </div>
                  )}
                </>
      ...
      <div className="bg-gradient-to-r from-indigo-50 to-indigo-100 dark:from-indigo-900/20 dark:to-indigo-900/10 p-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex items-center">
          ...
          <p className="text-indigo-700 dark:text-indigo-300 text-sm">
            Data reflects usage patterns of your team members
          </p>
        </div>
      </div>
    </motion.div>
...
export default SubordinateUsageSection;
