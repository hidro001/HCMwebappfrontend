...
// /**
//  * This array is the same as in EmployeeRatingAdvanced,
//  * with the icon-based frequency buttons.
//  */
// const FREQUENCIES = [
...
//  * (same logic as your manager component).
//  */
...
// /**
//  * Identical function to render the date/period for a rating doc
//  */
...
//     ];
//     const index = parseInt(rdoc.month, 10) - 1;
//     const name = monthNames[index] || `Month ${rdoc.month}`;
//     return `${name} ${rdoc.year}`;
...
// /**
//  * This component has the EXACT same UI as "EmployeeRatingAdvanced",
//  * but it doesn't pass employeeId. Instead it calls a new endpoint
//  * /ratings/my-advanced which reads the userId from the JWT token.
//  */
// function MyPerformanceAdvanced() {
//   // We'll store the same UI states for filters
//   const [isFilterExpanded, setIsFilterExpanded] = useState(false);
//   const [activeTab, setActiveTab] = useState("ratings"); // 'ratings' or 'charts'

//   // from your Zustand store, we want to call getMyRatingsAdvanced
//   // or a similar function that calls /ratings/my-advanced
//   const { getMyRatingsAdvanced, loading, error } = useRatingStore();
...
//   // EXACT same logic to reset frequency defaults
//   useEffect(() => {
//     switch (frequency) {
//       case "daily":
...
//       if (y >= 0 && m >= 0) {
//         const wArr = getWeeksInMonth(y, m);
//         setStartAvailableWeeks(wArr);
//         if (!startWeek || !wArr.find((w) => w.value === startWeek)) {
//           setStartWeek(wArr[0]?.value || "");
//         }
//       } else {
//         setStartAvailableWeeks([]);
//       }
...
//       if (y >= 0 && m >= 0) {
//         const wArr = getWeeksInMonth(y, m);
//         setEndAvailableWeeks(wArr);
//         if (!endWeek || !wArr.find((w) => w.value === endWeek)) {
//           setEndWeek(wArr[0]?.value || "");
//         }
//       } else {
//         setEndAvailableWeeks([]);
//       }
...
//   // This time we do NOT pass an employeeId at all
//   // We just call getMyRatingsAdvanced, which does /ratings/my-advanced
//   // The backend extracts userId from the JWT
...
//       // fetch from new endpoint: /ratings/my-advanced
//       const res = await getMyRatingsAdvanced(params);
//       if (res.success) {
...
//         toast.success(`Found ${filteredRatings.length} rating(s) for you`);
//       } else {
//         toast.error(res.message || "Could not fetch your performance data");
//       }
//     } catch (err) {
...
//   // same KPI detail modal logic
//   const handleOpenModal = (doc) => {
//     setSelectedRating(doc);
//     setShowModal(true);
//   };
...
//         {/* Dashboard header - EXACT same design */}
...
//         {/* Filter section (same as manager) */}
...
//               {/* Daily / Weekly / Monthly / Yearly filters (same UI) */}
...
//         {/* Error message (same style as manager) */}
//         {error && (
...
//         {/* Data panel: same design, but it's the logged-in user's data now */}
//         {employeeData && (
...
//             {/* Tabs: Ratings or Charts (identical to manager) */}
...
//                         No performance rating documents match your selected filters. 
...
//                                 <div className="flex items-center">
//                                   <span
//                                     className={`font-bold ${getScoreColor(rdoc.totalScore)}`}
//                                   >
...
//       {/* KPI detail modal, same as manager code */}
//       <KPIDetailModal
//         showModal={showModal}
...
// export default MyPerformanceAdvanced;
