...
export default function AllEmployeeRatings() {
  const navigate = useNavigate();

  // ======== STORES & ACTIONS ========
  const { getOrgRatingsAdvancedAggregated, loading, error } = useRatingStore();
  const {
    departments,
    fetchDepartments,
    loading: deptLoading,
  ...
  // load depts & designations
  useEffect(() => {
    fetchDepartments();
    fetchDesignations();
  }, [fetchDepartments, fetchDesignations]);

  // reset fields on frequency change
  useEffect(() => {
    const today = new Date();
    const isoToday = today.toISOString().split("T")[0];
    const y  = today.getFullYear();
    ...
    switch (frequency) {
      case "daily":
        ...
        setStartAvailableWeeks([]); setEndAvailableWeeks([]);
        break;
      case "weekly":
        setStartDate(""); setEndDate("");
        setStartYear(String(y)); setEndYear(String(y));
        setStartMonth(String(mm).padStart(2,"0"));
        setEndMonth(String(mm).padStart(2,"0"));
        setStartWeek(""); setEndWeek("");
        break;
  ...
  // recalc weekly start weeks
  useEffect(() => {
    if (frequency !== "weekly") return;
    if (startYear && startMonth) {
      const y = +startYear, m = +startMonth - 1;
      const weeks = getWeeksInMonth(y, m);
      setStartAvailableWeeks(weeks);
      if (!weeks.find(w => w.value === startWeek))
        setStartWeek(weeks[0]?.value || "");
    } else {
      setStartAvailableWeeks([]);
    }
  ...
  // recalc weekly end weeks
  useEffect(() => {
    if (frequency !== "weekly") return;
    if (endYear && endMonth) {
      const y = +endYear, m = +endMonth - 1;
      const weeks = getWeeksInMonth(y, m);
      setEndAvailableWeeks(weeks);
      if (!weeks.find(w => w.value === endWeek))
        setEndWeek(weeks[0]?.value || "");
    } else {
      setEndAvailableWeeks([]);
    }
      ...
      <div className="max-w-7xl mx-auto p-6">
            ...
            <div className="p-5 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <select
                    className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    value={department}
                    onChange={e => setDepartment(e.target.value)}
                  >
                    <option value="">-- All --</option>
                  ...
                  {deptLoading && <p className="text-sm text-gray-400 mt-1">Loading departments...</p>}
                  {deptError   && <p className="text-sm text-red-400 mt-1">{deptError}</p>}
                </div>
                {/* Designation */}
                <div>
                  <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  {desigLoading && <p className="text-sm text-gray-400 mt-1">Loading designations...</p>}
                  {desigError   && <p className="text-sm text-red-400 mt-1">{desigError}</p>}
                </div>
                {/* Sort By */}
                <div>
                  <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">Sort By</label>
                  <select
                    className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    value={sortBy}
                    onChange={e => setSortBy(e.target.value)}
              ...
              <div className="space-y-4">
                {frequency === "daily" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">Start Date</label>
                      <input
                        type="date"
                        className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        value={startDate}
                    ...
                    <div>
                      <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">End Date</label>
                      <input
                        type="date"
                        className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        value={endDate}
                ...
                {frequency === "weekly" && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div>
                        <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">Start Year</label>
                        <input
                          type="number"
                          placeholder="2025"
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      ...
                      <div>
                        <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">Start Month</label>
                        <select
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          value={startMonth}
                          onChange={e => setStartMonth(e.target.value)}
                      ...
                      <div>
                        <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">Start Week</label>
                        <select
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          value={startWeek}
                          onChange={e => setStartWeek(e.target.value)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div>
                        <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">End Year</label>
                        <input
                          type="number"
                          placeholder="2025"
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      ...
                      <div>
                        <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">End Month</label>
                        <select
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          value={endMonth}
                          onChange={e => setEndMonth(e.target.value)}
                      ...
                      <div>
                        <label className="block font-medium text-gray-700 dark:text-gray-300 mb-2">End Week</label>
                        <select
                          className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                          value={endWeek}
                          onChange={e => setEndWeek(e.target.value)}
              ...
              <div>
                <button
                  onClick={handleFetchRatings}
                  className="flex items-center justify-center px-6 py-3 space-x-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-lg font-medium shadow-md hover:shadow-lg transition-all duration-300"
                >
                  <FiSearch className="text-lg" />
                  <span>Generate Report</span>
                </button>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
          <div className="p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {orgData.length > 0 ? `${orgData.length} employees found` : "No data yet"}
            </div>
          </div>
          <div className="p-5">
            {orgData.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500 max-w-md">
                  Select filters above and click "Generate Report" to view organization-wide performance analytics.
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                ...
                <div className="mb-4">
                  <input
                    ...
                    className="w-full md:w-1/3 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
                <table className="min-w-full">
                  <thead>
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {orgData
                      ...
                      .map(({ employee, averageRating, ratingCount, category }) => {
                        const params = new URLSearchParams();
                        params.append("periodType", frequency);
                        if (frequency === "daily") {
                          params.append("startDate", startDate);
                        ...
                        return (
                          <tr key={employee._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors duration-150">
                            <td className="px-6 py-4">
                              <div className="flex items-center space-x-3">
                                <img
                                  src={employee.user_Avatar || "/placeholder-avatar.svg"}
                            ...
                            <td className="px-6 py-4">{employee.designation || "—"}</td>
                            <td className="px-6 py-4">{ratingCount || 0}</td>
                            <td className="px-6 py-4">{averageRating ? averageRating.toFixed(2) : "0.00"}</td>
