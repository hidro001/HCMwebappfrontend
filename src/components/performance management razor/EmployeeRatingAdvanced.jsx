...
// /**
//  * Helper to render the date/period for a rating doc
//  * (NOT used anymore for aggregated periods, but kept for KPI modal)
//  */
// function renderPeriod(rdoc) {
//   if (rdoc.frequency === "daily") {
//     if (!rdoc.date) return "No date";
...
//   // -----------------------------------------------------
//   // 2. NEW: State to hold aggregated data (instead of "filteredRatings")
//   // -----------------------------------------------------
//  const [aggregatedData, setAggregatedData] = useState([]);

//   // Keep employee info + average/ratingCount
//   const [employeeData, setEmployeeData] = useState(null);
...
//   // -----------------------------------------------------
//   // 3) Change handleFetchRatings() to use getEmployeeAggregatedRatings
//   // -----------------------------------------------------
//   const handleFetchRatings = async () => {
//     if (!employeeId) {
//       toast.error("No employee selected.");
...
//         // periodType must be one of: "daily","weekly","monthly","yearly"
//         // (you could later add "quarterly", "halfyearly", etc. in the future)
//       };
...
//         params.endYear = endYear;
//       }
// // Inside handleFetchRatings:

// const res = await getEmployeeAggregatedRatings(employeeId, params);

// if (res.success) {
//   const { employee, aggregatedBy, data } = res.data;
//   // data is an array of:
//   // { periodLabel, periodStartDate, periodEndDate, summary: { totalTarget, totalAchieved, totalScore, perKpi, percentOfTarget, category } }

//   setEmployeeData({
//     ...employee,
//     aggregatedBy,
//     ratingCount: data.length,
//     // (if you want an “averageRating,” you could compute it here, e.g.
//     //  average over data[].summary.percentOfTarget. For now, we just use count.)
//   });
//   setAggregatedData(data);
//   toast.success(`Found ${data.length} periods`);
// } else {
//   toast.error(res.message || "Could not fetch data");
// }
//     } catch (err) {
...
//   // handler to open KPI detail modal (unchanged, still shows individual period’s KPI breakdown)
//   const handleOpenModal = (ratingDoc) => {
//     setSelectedRating(ratingDoc);
//     setShowModal(true);
//   };
...
//                       </h3>
//                       <p className="text-gray-500 dark:text-gray-400 max-w-md">
//                         No performance data matches your filters. Try adjusting
//                         your period or selecting a different time range.
//                       </p>
//                     </div>
//                   ) : (
//                     <div className="overflow-x-auto">
...
//                         </thead>
//                         <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
//                           {aggregatedData.map((periodObj) => {
//                             const { periodLabel, summary } = periodObj;
//                             return (
//                               <tr
//                                 key={periodLabel}
//                                 className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
...
//                                       summary.category === "Outstanding"
//                                         ? "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300"
//                                       : summary.category === "Exceeds Expectations"
//                                         ? "bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300"
//                                       : summary.category === "Meets Expectations"
//                                         ? "bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300"
//                                       : summary.category === "Needs Improvement"
...
// /**
//  * Helper to render the date/period for a rating doc
//  * (NOT used anymore for aggregated rows, but kept for KPI modal)
//  */
// function renderPeriod(rdoc) {
//   if (rdoc.frequency === "daily") {
//     if (!rdoc.date) return "No date";
...
//   // -----------------------------------------------------
//   // State to hold aggregated data instead of raw ratings
//   // -----------------------------------------------------
//   const [aggregatedData, setAggregatedData] = useState([]);

//   // Keep employee info + ratingCount (number of periods returned) + aggregatedBy + averageRating
//   const [employeeData, setEmployeeData] = useState(null);

//   // We also need to keep the raw daily‐rating documents (filteredRatings) so that the modal can show per‐KPI details
...
//       if (res.success) {
//         // The new API returns: { success: true, data: { employee, aggregatedBy, aggregatedData, filteredRatings, averageRating, ratingCount } }
//         const {
//           employee,
//           aggregatedBy,
...
//   // handler to open KPI detail modal (passes entire period object so modal can show perKpi)
//   const handleOpenModal = (periodObj) => {
//     // periodObj.periodLabel is e.g. “2025-05-27”.
//     // The raw rating docs have their “date” in ISO string form like “2025-05-27T00:00:00.000Z”.
//     // Let’s find the single raw document in rawRatings whose date (slice to YYYY-MM-DD) matches periodLabel.
...
//   const handleOpenModal = (periodObj) => {
//     if (frequency === "weekly") {
//       // WEEKLY MODE: use aggregated per‐KPI sums directly from periodObj.summary.perKpi
//       // periodObj.summary.perKpi is an array like:
//       //   [ { kpiName, type:"quantitative", targetSum, achievedSum }, 
//       //     { kpiName, type:"qualitative", marksSum, scoreSum }, … ]
//       const aggregatedPerKpi = periodObj.summary.perKpi || [];
//       setSelectedPeriod(periodObj);
//       setSelectedRawRating(aggregatedPerKpi);
//       setShowModal(true);
//     } else {
//       // NON-WEEKLY: find a single raw daily‐rating doc whose date matches periodLabel
//       const matchingRaw = rawRatings.find((r) => {
//         if (!r.date) return false;
//         const yyyyMmDd = new Date(r.date).toISOString().slice(0, 10);
//         return yyyyMmDd === periodObj.periodLabel;
...
//                                   <button
//                                     className="inline-flex items-center px-3 py-1 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 rounded-md transition-colors text-sm font-medium"
//                                     onClick={() => handleOpenModal(periodObj)}
//                                   >
//                                     <FiEye className="mr-1" />
//                                     View KPIs
//                                   </button>
...
//   // handler to open KPI detail modal (passes entire period object so modal can show perKpi)
//   const handleOpenModal = (periodObj) => {
//     // ** CHANGE HERE: “any non‐daily frequency” now uses summary.perKpi.
//     if (frequency !== "daily") {
...
//         return d >= periodObj.periodStartDate && d < periodObj.periodEndDate;
//       }).length;
...
//   // We also need to keep the raw daily‐rating documents (filteredRatings)
//   // so that the modal can show per‐KPI details
//   const [rawRatings, setRawRatings] = useState([]);
...
//   const handleOpenModal = (periodObj) => {
//     // ** For any non‐daily frequency, use summary.perKpi + compute maxScore dynamically.
//     if (frequency !== "daily") {
//       // 1) Use the aggregated per‐KPI sums directly from periodObj.summary.perKpi
//       const aggregatedPerKpi = periodObj.summary.perKpi || [];
...
//         // Convert the API’s ISO strings into actual Date objects:
...
// src/components/EmployeeRatingAdvanced.jsx
                      ...
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-100 dark:divide-gray-700">
                          {aggregatedData.map((periodObj) => {
                            ...
                            // Calculate daysCount & maxScore for this period on the fly
                            const daysCount = rawRatings.filter((r) => {
                              if (!r.date) return false;
                              const d = new Date(r.date);
                              const start = new Date(periodObj.periodStartDate);
                            ...
                            // Determine color band based on pctRounded
                            const colorClass = getScoreColor(pctRounded);
