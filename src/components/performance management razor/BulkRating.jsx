...
function BulkRating() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // If the API returns success, but with partial row errors
      // example response:
      // {
      //   success: true,
      //   data: {
      ...
      //     errorCount: 1,
      //     errors: [
      //       { rowNumber: 3, errors: ["Score cannot exceed Max Marks"] }
      //     ]
      //   }
      // }
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-950 text-gray-800 dark:text-gray-100 rounded-2xl">
      <div className="mx-auto">
        ...
        {/* Show loading spinner if request is in-flight */}
        {loading && (
        ...
        {/* Show an error for designations if needed */}
        {designationError && (
        ...
        {/* NEW: Show partial errors from API in a heading/alert if any */}
        {errorCount > 0 && (
          <div className="mb-4 p-4 border-l-4 border-red-500 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-md">
            <h3 className="font-bold text-red-600 dark:text-red-300">
              We found {errorCount} error{errorCount > 1 ? "s" : ""} in your file:
            </h3>
            <ul className="list-disc list-inside mt-2">
              {bulkErrors.map((errObj, idx) => (
                <li key={idx}>
                  Row {errObj.rowNumber}: {errObj.errors.join(", ")}
                </li>
              ))}
            </ul>
          </div>
