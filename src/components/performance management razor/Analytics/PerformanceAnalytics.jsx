...
// // A refreshed color palette (you can swap these around as desired)
// const COLORS = [
//   "#2B6CB0",
//   "#38A169",
//   "#E53E3E",
...
// // Utility: Given a percentage (0–100) return a color class if you want to do background, etc.
// // (Not directly used below but available if you want to add colored backgrounds to cards.)
// function getScoreColorClass(scoreOutOf100) {
...
// }) {
//   //
//   // 1) Build chartData for each period (used by multiple charts).
//   //
//   const chartData = useMemo(() => {
//     return aggregatedData.map((pd) => {
//       const { periodLabel, summary, periodStartDate, periodEndDate } = pd;

//       // Count how many raw daily docs fall into this period → maxScore = daysCount * 100
...
//       // normalizedScore = totalScore / maxScore * 100 (if maxScore > 0)
...
//   //
//   // 3) RadarChart: average % per KPI (across all aggregated periods)
//   //
//   const kpiRadarData = useMemo(() => {
//     // Map: { kpiName: { sumRatio, occurrences } }
//     const map = {};
//     aggregatedData.forEach((pd) => {
...
//         } else {
//           // qualitative: scoreSum / marksSum * 100
...
//   //
//   // 4) ScatterChart: each period’s (totalTarget, totalAchieved)
//   //
//   const scatterData = useMemo(() => {
//     return aggregatedData.map((pd) => ({
//       period: pd.periodLabel,
//       target: pd.summary.totalTarget,
//       achieved: pd.summary.totalAchieved,
//     }));
//   }, [aggregatedData]);

//   //
//   // 5) Average daily score per KPI (new chart): iterate rawRatings
//   //
//   const avgDailyKpiData = useMemo(() => {
//     // Map: { kpiName: { dailySum: { dateKey: totalScoreForThatKpiOnThatDate }, uniqueDates: Set } }
//     // We want: for each KPI, sum scores *per day* and then average across days on which it appeared.
//     // But each rawRating is a daily doc containing multiple KPIs. We'll accumulate:
//     //   map[kpiName].sumScores += score,
//     //   map[kpiName].countDates = number of distinct days it appeared.
//     const map = {};
//     rawRatings.forEach((dr) => {
//       if (!dr.date) return;
...
//   //
//   // 6) Normalized Score Distribution histogram (0–10, 10–20, … 90–100)
//   //
//   const normalizedBuckets = useMemo(() => {
//     // Build buckets: “0–10”, “10–20”, …, “90–100”
//     const buckets = Array.from({ length: 10 }).map((_, i) => ({
//       range: `${i * 10}–${i * 10 + 10}%`,
//       count: 0,
//     }));
//     chartData.forEach((d) => {
//       const pct = d.normalizedScore;
//       let idx = Math.min(Math.floor(pct / 10), 9);
//       buckets[idx].count += 1;
//     });
//     return buckets;
//   }, [chartData]);
//   const kpiBarData = kpiRadarData.map((d) => ({
//     kpiName: d.kpiName,
//     avgPercent: d.avgPercent,
//   }));
//   return (
//     <div className="space-y-12">
//       {/* ──────────────────────────────────────────────────────── */}
//       {/* 1) LineChart: Total Score & % of Target Over Time */}
//       <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//         <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
//           Score &amp; % of Target Over Time
//         </h4>
//         {chartData.length === 0 ? (
...
//                   if (name === "% of Target") return [`${value}%`, name];
//                   return [value, name];
//                 }}
//               />
...
//       {/* ──────────────────────────────────────────────────────── */}
//       {/* 2) BarChart: Total Target vs. Total Achieved per Period */}
//       <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//         <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
//           Total Target vs. Total Achieved
//         </h4>
//         {chartData.length === 0 ? (
//           <div className="text-center text-sm text-gray-500 dark:text-gray-400">
//             No data to display.
...
//       {/* ──────────────────────────────────────────────────────── */}
//       {/* 3) AreaChart: Normalized Score Trend (0–100)             */}
//       <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//         <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
//           Normalized Score (0–100) Over Time
//         </h4>
//         {chartData.length === 0 ? (
//           <div className="text-center text-sm text-gray-500 dark:text-gray-400">
//             No data to display.
...
//             >
//               <defs>
//                 <linearGradient id="colorNormGrad" x1="0" y1="0" x2="0" y2="1">
...
//       {/* ──────────────────────────────────────────────────────── */}
//       {/* 5) PieChart: Category Distribution (count of periods) */}
//       <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//         <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
//           Periods by Category
//         </h4>
...
//                 ))}
//               </Pie>
//               <Tooltip formatter={(val) => [val, "Periods"]} />
...
//       {/* ──────────────────────────────────────────────────────── */}
//       {/* 6) Horizontal BarChart: Average % by KPI Name */}
//       <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//         <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
...
//               {/* XAxis is the numeric axis (0–100%) */}
//               <XAxis
//                 type="number"
//                 domain={[0, 100]}
...
//                 fill="#2B6CB0"
//                 barSize={20}
//                 label={{ position: "right", fill: "#2B6CB0", fontSize: 12 }}
//               />
//             </BarChart>
//           </ResponsiveContainer>
//         )}
...
//  {/* ──────────────────────────────────────────────────────── */}
// {/* 7) LineChart: Total Target vs. Total Achieved Over Time */}
// <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//   <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
//     Total Target vs. Total Achieved Over Time
//   </h4>
//   {chartData.length === 0 ? (
//     <div className="text-center text-sm text-gray-500 dark:text-gray-400">
//       No data to display.
...
//       {/* ──────────────────────────────────────────────────────── */}
//       {/* 9) NEW! BarChart: Average Daily Score per KPI */}
//       <div className="bg-white dark:bg-gray-800 rounded-lg p-4 shadow">
//         <h4 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">
//           Average Daily Score per KPI
//         </h4>
