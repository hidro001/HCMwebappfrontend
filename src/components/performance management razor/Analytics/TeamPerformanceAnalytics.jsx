...
const COLORS = [
  ...
  "#3B82F6", "#F43F5E", "#8B5CF6", "#10B981", "#F59E0B"
];
...
const ScatterTooltip = ({ active, payload }) => {
  ...
  return (
    ...
    >
      <p className="font-semibold mb-2 text-gray-900 dark:text-white">{period}</p>
      <div className="space-y-1">
        <p className="text-gray-700 dark:text-gray-300">Target: <span className="font-medium">{target}</span></p>
        <p className="text-gray-700 dark:text-gray-300">Achieved: <span className="font-medium">{achieved}</span></p>
      </div>
    </motion.div>
  );
};

const EmptyState = ({ icon: Icon, title = "No data available", description = "Data will appear here once available" }) => (
  <motion.div
...
const ChartCard = ({ 
  ...
  >
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Interactive analytics</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          {actions}
    ...
    <div className="p-4 sm:p-6" style={{ height: isExpanded ? "32rem" : "24rem" }}>
...
const ResponsiveTable = ({ headers, rows, rowRenderer, searchable = false }) => {
...
export default function PerformanceAnalytics({ data, onRefresh }) {
  ...
  const performanceTrend = chart.length >= 2 
    ? chart[chart.length - 1].percentOfTarget - chart[chart.length - 2].percentOfTarget
    : 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      >
        ...
        <motion.div 
          variants={cardVariants}
          className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 p-6 sm:p-8"
        >
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div className="flex-1">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <div>
                  <h1 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-gray-900 via-gray-700 to-gray-600 dark:from-white dark:via-gray-200 dark:to-gray-400 bg-clip-text text-transparent">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
              ...
              <motion.button
                ...
                disabled={isLoading}
                className="flex items-center gap-3 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 text-white px-6 py-3 rounded-xl font-medium shadow-lg transition-all duration-200 disabled:opacity-50"
              >
                {isLoading ? <LoadingSpinner /> : <FiRefreshCw className="w-5 h-5" />}
                Refresh Data
              </motion.button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          <StatCard
            ...
            trend={performanceTrend > 0 ? "up" : performanceTrend < 0 ? "down" : "stable"}
          ...
          <StatCard
            ...
            trendValue={`${avgPerf >= 80 ? "Excellent" : avgPerf >= 60 ? "Good" : "Needs Improvement"}`}
                ...
                <BarChart data={chart} margin={{ top: 20 }}>
                  <defs>
                    <linearGradient id="targetBar" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <stop offset="95%" stopColor={COLORS[4]} stopOpacity={0.6} />
                    </linearGradient>
                    <linearGradient id="achievedBar" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={COLORS[3]} stopOpacity={0.8} />
                      <stop offset="95%" stopColor={COLORS[3]} stopOpacity={0.6} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" opacity={0.5} />
                  <XAxis 
                    ...
                    content={({ payload }) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 text-sm max-w-xs"
                        >
                          <p className="font-semibold mb-2 text-gray-900 dark:text-white">{category}</p>
                          <p className="text-gray-700 dark:text-gray-300 mb-2">
                            {value} period{value > 1 ? "s" : ""}
                          </p>
                          {periods.length > 0 && (
                            <>
                              <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">Periods:</p>
                ...
                >
                  <defs>
                    <linearGradient id="kpiBar" x1="0" y1="0" x2="1" y2="0">
                      ...
                      <stop offset="95%" stopColor={COLORS[5]} stopOpacity={0.6} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" opacity={0.5} />
                  <XAxis
                    ...
                    content={({ payload }) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl p-4 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 text-sm max-w-xs"
                        >
                          <p className="font-semibold mb-2 text-gray-900 dark:text-white">{range}</p>
                          <p className="text-gray-700 dark:text-gray-300 mb-2">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={mvData}>
                  ...
                  <Bar 
                    ...
                    name="% Change"
                    fill={(entry) => entry > 0 ? "url(#changeBarPositive)" : "url(#changeBarNegative)"}
                    radius={[4, 4, 0, 0]}
                  >
                    <LabelList
                      dataKey="periodChange"
              ...
              >
                <defs>
                  <linearGradient id="dailyKpiBar" x1="0" y1="0" x2="0" y2="1">
                    ...
                    <stop offset="95%" stopColor={COLORS[6]} stopOpacity={0.6} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#E5E7EB" opacity={0.5} />
                <XAxis
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Employee Performance</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Comprehensive employee metrics</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <ResponsiveTable
                ...
                rowRenderer={(e) => [
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-medium">{`${e.employee?.first_Name || ''} ${e.employee?.last_Name || ''}`}</span>
                  </div>,
                  <div className="flex items-center space-x-2">
                    ...
                    <div className={`px-2 py-1 rounded-full text-xs ${
                      ...
                      {e.averageRating >= 4 ? 'Excellent' : e.averageRating >= 3 ? 'Good' : 'Needs Improvement'}
                    </div>
                  </div>,
                  e.ratingCount,
                  <span className="px-2 py-1 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 rounded-full text-xs font-medium">{e.category}</span>
                ]}
                searchable={true}
              />
            </div>
            ...
            >
              <div className="p-6 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Top Performers</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Highest rated team members</p>
                  </div>
                </div>
              </div>
              <div className="p-6">
                <ResponsiveTable
                  ...
                  rowRenderer={(p) => [
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl font-bold text-green-600 dark:text-green-400">{p.averageRating}</span>
                      <FiTrendingUp className="w-4 h-4 text-green-500" />
                    </div>,
                    <span className="px-3 py-1 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 rounded-full text-xs font-medium">{p.category}</span>
                  ]}
                />
              </div>
            </motion.div>
            ...
            >
              <div className="p-6 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Needs Improvement</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Focus areas for development</p>
                  </div>
                </div>
              </div>
              <div className="p-6">
                <ResponsiveTable
                  ...
                  rowRenderer={(p) => [
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <div className="font-medium">{`${p.employee?.first_Name || ''} ${p.employee?.last_Name || ''}`}</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">Development focus</div>
                      </div>
                    </div>,
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl font-bold text-red-600 dark:text-red-400">{p.averageRating}</span>
                      <FiTrendingDown className="w-4 h-4 text-red-500" />
                    </div>,
                    <span className="px-3 py-1 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 rounded-full text-xs font-medium">{p.category}</span>
                  ]}
                />
              </div>
            </motion.div>
            ...
            <div className="p-6 border-b border-gray-100 dark:border-gray-800">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Performance Heatmap</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Employee performance across periods</p>
                  </div>
                </div>
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2 text-sm">
                    <div className="w-3 h-3 bg-red-100 dark:bg-red-900 rounded"></div>
                    <span className="text-gray-600 dark:text-gray-400">0-39%</span>
                  </div>
                  <div className="flex items-center space-x-2 text-sm">
                  ...
                  <div className="min-w-full">
                    <div className="grid grid-cols-[200px_1fr] gap-0 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
                      ...
                      <div className="bg-gray-50 dark:bg-gray-800 p-4 border-r border-gray-200 dark:border-gray-700">
                        <span className="text-sm font-semibold text-gray-700 dark:text-gray-300">Employee</span>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-800 p-2 grid gap-1" style={{ gridTemplateColumns: `repeat(${chart.length}, minmax(80px, 1fr))` }}>
                        {chart.map((c, i) => (
                          <div key={i} className="text-center p-2">
                            <span className="text-xs font-medium text-gray-700 dark:text-gray-300 transform -rotate-45 inline-block">{c.period}</span>
                          </div>
                        ))}
                      </div>
                        ...
                        <React.Fragment key={i}>
                          <div className="p-4 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 flex items-center">
                            <div className="flex items-center space-x-3">
                              ...
                              <span className="text-sm font-medium text-gray-900 dark:text-white">{row.employee}</span>
                            </div>
                          </div>
                          <div className="p-2 bg-white dark:bg-gray-900 grid gap-1" style={{ gridTemplateColumns: `repeat(${chart.length}, minmax(80px, 1fr))` }}>
                            {row.data.map((cell, j) => {
        ...
        <motion.div 
          variants={cardVariants}
          className="bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl rounded-3xl shadow-lg border border-gray-200/50 dark:border-gray-700/50 p-6 text-center"
        >
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
              <span>© 2025 Performance Analytics Dashboard</span>
              <span>•</span>
