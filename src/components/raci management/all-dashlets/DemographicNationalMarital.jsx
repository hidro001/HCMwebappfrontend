...
export default function DemographicNationalMarital() {
  const { data, loading, error, fetchNationalityMarital } =
    useNationalityMaritalStore();
  ...
  const totalNationality =
    nationality.Indian + nationality.US + nationality.Other;
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (context) {
            const percentage = (
              (context.parsed / totalNationality) *
              100
            ).toFixed(1);
  ...
  const barData = [
    {
      status: "Single",
      count: maritalDistribution.Single || 0,
      color: "#EF4444",
      icon: FiUser,
    },
    {
      status: "Married",
      count: maritalDistribution.Married || 0,
      color: "#3B82F6",
      icon: FiHeart,
    },
    {
      status: "Divorced",
      count: maritalDistribution.Divorced || 0,
      color: "#10B981",
      icon: FiUserCheck,
    },
  ];
  ...
  const maritalDistributionWithPct = barData.map((item) => ({
    ...item,
    percentage: totalMarital
      ? ((item.count / totalMarital) * 100).toFixed(1)
      : 0,
  }));
        ...
        >
          ...
          <div className="relative flex flex-col items-center h-40 sm:h-48 lg:h-52 mb-3 sm:mb-4">
            <Doughnut
              ...
              onClick={() => drill.fetch("nationality")}
            />
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
