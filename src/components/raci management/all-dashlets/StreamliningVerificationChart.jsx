...
const StreamliningVerificationChart = () => {
  ...
  const getDepartmentColor = (department) => {
    const colors = {
      'HR': { primary: '#3B82F6', secondary: '#60A5FA', gradient: 'from-blue-500 to-blue-600' },
      'IT': { primary: '#10B981', secondary: '#34D399', gradient: 'from-emerald-500 to-emerald-600' },
      'Finance': { primary: '#F59E0B', secondary: '#FBBF24', gradient: 'from-amber-500 to-amber-600' },
      'Operations': { primary: '#8B5CF6', secondary: '#A78BFA', gradient: 'from-violet-500 to-violet-600' },
      'Marketing': { primary: '#EF4444', secondary: '#F87171', gradient: 'from-red-500 to-red-600' },
      'Sales': { primary: '#06B6D4', secondary: '#22D3EE', gradient: 'from-cyan-500 to-cyan-600' },
      'Legal': { primary: '#84CC16', secondary: '#A3E635', gradient: 'from-lime-500 to-lime-600' },
      'Security': { primary: '#F97316', secondary: '#FB923C', gradient: 'from-orange-500 to-orange-600' },
      'Admin': { primary: '#EC4899', secondary: '#F472B6', gradient: 'from-pink-500 to-pink-600' },
      'Production': { primary: '#6366F1', secondary: '#818CF8', gradient: 'from-indigo-500 to-indigo-600' },
    };
  ...
  const calculateStats = () => {
    if (!data || data.length === 0 || !selectedDepartment) return { total: 0, cleared: 0, pending: 0, efficiency: 0 };
    
    const dept = data.find(d => d.department === selectedDepartment);
    if (!dept) return { total: 0, cleared: 0, pending: 0, efficiency: 0 };
    
    const total = dept.backgroundCleared + dept.backgroundPending + dept.policeCleared + dept.policePending;
  ...
  const getChartData = () => {
    ...
    return {
      ...
      datasets: [
        {
          label: "Cleared",
          data: [selectedDeptData.backgroundCleared, selectedDeptData.policeCleared],
        ...
        {
          label: "Pending",
          data: [selectedDeptData.backgroundPending, selectedDeptData.policePending],
  ...
  const filterOptions = [
    ...
    { value: 'high-performance', label: 'High Performance', icon: FiTrendingUp },
      ...
      >
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center space-y-4">
            ...
            />
            <p className="text-slate-600 dark:text-slate-400 font-medium">Loading verification insights...</p>
          </div>
        </div>
      </motion.div>
    );
      ...
      >
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-red-600 dark:text-red-400 font-semibold">Unable to load verification data</p>
      ...
      >
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-slate-600 dark:text-slate-400 font-medium">No verification data available</p>
            <p className="text-slate-500 dark:text-slate-500 text-sm mt-2">Data will appear here once available</p>
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    ...
    >
      ...
      <div className="relative p-6 pb-4">
            ...
            <div className="relative">
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: selectedDepartment ? getDepartmentColor(selectedDepartment).primary : '#64748B' }}
                />
                <span className="flex-1 text-left truncate">{selectedDepartment || 'Select Department'}</span>
                <motion.div
                    ...
                    {data?.map((dept, index) => (
                      <motion.button
                        ...
                        className={`w-full flex items-center space-x-3 px-4 py-3 text-sm text-left hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${
                          selectedDepartment === dept.department ? 'bg-blue-50 dark:bg-blue-900/20' : ''
                        }`}
                        whileHover={{ x: 4 }}
                      ...
                      >
                        <div
                          className="w-3 h-3 rounded-full shadow-sm"
                          style={{ backgroundColor: getDepartmentColor(dept.department).primary }}
                        />
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                            {(dept.backgroundCleared + dept.policeCleared + dept.backgroundPending + dept.policePending)} total cases
                          </div>
                        </div>
                        {selectedDepartment === dept.department && (
        ...
        >
          {[
            { label: "Total Cases", value: stats.total, icon: FiShield, color: "from-blue-500 to-blue-600" },
            { label: "Cleared", value: stats.cleared, icon: FiCheckCircle, color: "from-emerald-500 to-emerald-600" },
            { label: "Pending", value: stats.pending, icon: FiClock, color: "from-amber-500 to-amber-600" },
            { label: "Efficiency", value: `${stats.efficiency}%`, icon: FiTrendingUp, color: "from-purple-500 to-purple-600" },
          ].map((stat, index) => (
            <motion.div
              key={stat.label}
      ...
      <div className="px-6 pb-4">
        ...
        >
          {[
            { label: "Background (Cleared)", color: "#3B82F6", bgColor: "rgba(59, 130, 246, 0.1)" },
            { label: "Background (Pending)", color: "#60A5FA", bgColor: "rgba(96, 165, 250, 0.1)" },
            { label: "Police (Cleared)", color: "#10B981", bgColor: "rgba(16, 185, 129, 0.1)" },
            { label: "Police (Pending)", color: "#22C55E", bgColor: "rgba(34, 197, 94, 0.1)" },
          ].map((item, index) => (
            <motion.div
              key={item.label}
              initial={{ opacity: 0, x: -10 }}
