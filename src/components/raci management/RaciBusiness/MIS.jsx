...
const MIS = ({ setScore }) => {
  const questions = [
    {
      id: 1,
      text: "Do you operate modern computer-based accounting and business systems?",
      icon: HiDesktopComputer,
      category: "System Infrastructure",
      description: "Modern technology foundation for business operations",
      systemType: "core"
    },
    {
      id: 2,
      text: "Do your systems report on sales, cost of goods, and gross margin?",
      icon: FaChartPie,
      category: "Financial Reporting",
      description: "Key financial metrics tracking and analysis capabilities",
      systemType: "analytics"
    },
    {
      id: 3,
      text: "Policies and procedures exist for at least monthly reporting.",
      icon: FaFileAlt,
      category: "Process Management",
      description: "Regular reporting governance and operational procedures",
      systemType: "governance"
    },
    {
      id: 4,
      text: "Existence and use of best practice operating systems.",
      icon: FaCogs,
      category: "Best Practices",
      description: "Implementation of industry-standard operational systems",
  ...
  const getSystemTypeColor = (type) => {
    const colors = {
      "core": "bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300",
      "analytics": "bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-300",
      "governance": "bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300",
      "optimization": "bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300"
    };
  ...
  const getCategoryColor = (category) => {
    const colors = {
      "System Infrastructure": "bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300",
      "Financial Reporting": "bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-300",
      "Process Management": "bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300",
      "Best Practices": "bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300"
    };
  ...
  const getCategoryIconColor = (category) => {
    const colors = {
      "System Infrastructure": "bg-blue-100 dark:bg-blue-900/20",
      "Financial Reporting": "bg-green-100 dark:bg-green-900/20",
      "Process Management": "bg-purple-100 dark:bg-purple-900/20",
      "Best Practices": "bg-orange-100 dark:bg-orange-900/20"
    };
  ...
  const getCategoryIconTextColor = (category) => {
    const colors = {
      "System Infrastructure": "text-blue-600 dark:text-blue-400",
      "Financial Reporting": "text-green-600 dark:text-green-400",
      "Process Management": "text-purple-600 dark:text-purple-400",
      "Best Practices": "text-orange-600 dark:text-orange-400"
    };
    return colors[category] || "text-gray-600 dark:text-gray-400";
  };

  const getSystemMaturityLevel = (score) => {
    if (score >= 90) return { level: "Advanced", color: "text-green-600 dark:text-green-400", icon: "🚀" };
    if (score >= 75) return { level: "Mature", color: "text-blue-600 dark:text-blue-400", icon: "⭐" };
    if (score >= 50) return { level: "Developing", color: "text-yellow-600 dark:text-yellow-400", icon: "🔧" };
    if (score >= 25) return { level: "Basic", color: "text-orange-600 dark:text-orange-400", icon: "⚠️" };
    return { level: "Limited", color: "text-red-600 dark:text-red-400", icon: "❌" };
  };
  ...
  const maturityLevel = getSystemMaturityLevel(averagePercentage);
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-3 bg-indigo-100 dark:bg-indigo-900/20 rounded-xl">
              <FaServer className="text-indigo-600 dark:text-indigo-400 text-2xl" />
            </div>
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Evaluate your business technology infrastructure and information systems
              </p>
            </div>
          </div>
          ...
          <div className="text-right">
            <div className="text-sm text-gray-500 dark:text-gray-400">System Maturity</div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-sm mb-2">
            <span className="text-gray-600 dark:text-gray-400">
              Assessment Progress: {completedQuestions}/{questions.length} systems
            </span>
            <span className="text-gray-600 dark:text-gray-400">
                    ...
                    >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-col space-y-2 mb-2">
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                <span className={`inline-flex px-2 py-1 text-xs rounded-full w-fit ${getSystemTypeColor(question.systemType)}`}>
                                  {question.systemType === "core" ? "Core System" :
                                   question.systemType === "analytics" ? "Analytics" :
                                   question.systemType === "governance" ? "Governance" :
                                   "Optimization"}
                                </span>
                              </div>
                            </div>
                      ...
                      <td className="py-4 px-4 align-top">
                        <select
                          value={value}
                          onChange={(e) => handleDropdownChange(index, e)}
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 text-sm"
                        >
                ...
                >
                  ...
                  <div className="flex items-start space-x-3 mb-4">
                    ...
                    <div className="flex-1">
                      <div className="flex flex-wrap gap-2 mb-2">
                        ...
                        <span className={`px-2 py-1 text-xs rounded-full ${getSystemTypeColor(question.systemType)}`}>
                          {question.systemType === "core" ? "Core System" :
                           question.systemType === "analytics" ? "Analytics" :
                           question.systemType === "governance" ? "Governance" :
                           "Optimization"}
                  ...
                  <div className="space-y-3">
                    ...
                    <select
                      value={value}
                      onChange={(e) => handleDropdownChange(index, e)}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200"
                    >
                      <option value="">Select status</option>
                      {options[index].map((option, optIndex) => (
                        <option key={optIndex} value={option}>
